...
_sys_timezone() {
	...
	echo "Have set your host to Asia/Shanghai timezone and automatically sync time via systemd-timesyncd。"
	echo
...
v2ray_config() {
	...
	while :; do
		...
		echo
		echo "Note 1: The dynamic port is enabled with [dynamicPort].."
    echo "Note 2: [utp | srtp | wechat-video | dtls | wireguard] respectively disguised as [BT download | video call | WeChat video call | DTLS 1.2 packet | WireGuard packet]"
		echo
...
v2ray_dynamic_port_start() {
	...
	while :; do
		echo -e "please enter "$yellow"V2Ray dynamic port start "$none"scope ["$magenta"1-65535"$none"]"
...
tls_config() {
	...
	while :; do
		echo
		echo -e "Please enter a ${magenta}correct domain name${none}, it must be correct, no! can! out! wrong!"
		read -p "(Example：123blog.com): " domain
	...
	while :; do
		...
		if [[ -z "$record" ]]; then
			...
			if [[ "$record" == [Yy] ]]; then
				...
				echo
				echo
				echo -e "$yellow Domain name resolution = ${cyan} I'm sure there is already a resolution $none"
				echo "----------------------------------------------------------------"
				echo
				break
			else
...
path_config_ask() {
	...
	while :; do
		...
		N | n)
			echo
			echo
			echo -e "$yellow site masquerading and routing = $cyan doesn't want to configure $none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
path_config() {
	echo
	while :; do
		echo -e "Please enter the path you want ${magenta} to use to divert $none , such as /233blog , then just enter 233blog"
		...
		*)
			echo
			echo
			echo -e "$yellow diverted path = ${cyan}/${path}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
proxy_site_config() {
    ...
    [[ -z $proxy_site ]] && proxy_site="https://liyafly.com"
		...
		*[#$]*)
			echo
			echo -e " Since this script is too spicy.. so the fake URL cannot contain the symbols $red # $none or $red $ $none. …"
			echo
			error
			;;
		*)
